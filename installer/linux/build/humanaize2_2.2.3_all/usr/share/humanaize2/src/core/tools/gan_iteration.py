...
# Add core directory to path for llm import
...
class GANIteration:
    ...
    def decide_use_gan(self, user_text, exec_instr, emotion_monitor=None):
        """
        決定助理在回答前是否應該執行 GAN 自我辯論。
        Returns:
            tuple(bool, str): 是否使用 GAN，以及決策說明。
        """
        self._reset_session()
        if self._check_stop():
            return False, ""
    ...
    def self_debate(self, is_user_topic=False, user_topic=None):
        ...
        GAN 迭代：兩個觀點之間的自我辯論。
        Args:
            is_user_topic (bool): 主題是否由使用者定義。
            user_topic (str): 如果 is_user_topic 為 True，則為使用者提供的主題。
        Returns:
            dict: {"synthesis": str} 包含最終的綜合結果。
        """
        self._reset_session()
        if self._check_stop():
            return {"synthesis": ""}
